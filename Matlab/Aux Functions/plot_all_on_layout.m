function [] = plot_all_on_layout(layout, lim, n_usrs, other_participants,...
    ...
    n_usr = n_usrs - size(other_participants, 1);
    ...
    for s=1:snapshot_interval:last_snapshot
        ...
        if floor(s/pause_snap_interval) > curr_divisor && ...
                mod(last_snapshot, s) >  pause_snap_interval
            % disp(s);
...
%     a = [];
%     for i = 1:length(f)
%         a = [a convertCharsToStrings(num2str(i))];
%     end
%     legend(f,a);
