...
def main(drop_rate_file, filename): # drop_rate, 
    ...
    for packet in _read_packets(pcap_reader):
        ...
        """
        # Successful packets
        # if total_packets % int(100/drop_rate) != 0:            
...
for i in range(len(csv_script.pdr)):  
    ...
    input_file = f'PDR_{csv_script.pdr[i]}%.csv'
    input_file_name = os.path.join(input_path, input_file)

    main(input_file_name, input_file)
