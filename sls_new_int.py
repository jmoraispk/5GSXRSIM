...
class Beam_pair():
    ...
    def __init__(self):
        ...
        # For saving the relative power indicator of each beam chosen for LC
        # precoder for checking the orthogobality of the final LC beam.
        self.RPI = []
...
def update_precoders(bs, ue, curr_beam_pairs, precoders_dict, curr_coeffs, 
    ...
    #Initiating these variables for saving criss cross values for both ranks.
    distribute_var_beam_idx = np.empty((n_layers, n_csi_beams))
    distribute_var_ch_power_gain = np.empty((n_layers, n_csi_beams))
    distribute_var_relative_beam_idx = np.empty((n_layers, n_csi_beams))
    #this variable is a test, for saving rank 1 chosen precoders and then removing 
    #them while sending the precoder list to for selection in rank 2.
    best_beam_pairs_list_rank1 = [] 
    ...
    for l in range(n_layers):
        ...
        #Below is commented because rank 2 takes beams from the same RF    
        # else:
        #     if subset_GoB:
        ...
        q_idxs_temp = q_idxs_l
        if l:
            q_idxs_l = set(q_idxs_l)
            # q_idxs_temp = q_idxs_l
            best_beam_pairs_list_rank1 = set(best_beam_pairs_list_rank1)
            q_idxs_l = list(q_idxs_l.difference(best_beam_pairs_list_rank1))
        ...
        amp_scaling_list = [1, np.sqrt(1/2), np.sqrt(1/4), \
                                np.sqrt(1/8), np.sqrt(1/16), np.sqrt(1/32), \
                                np.sqrt(1/64), 0]
        
            #Do the below steps for allowed rank = 1 for the entire simulation
            #The reason is, for rank 2, we need to first find only the beams and 
            # index for each layer and then we need to criss cross them or 
            #re-arrange them before combining, we can club them at a later stage
            #but for now lets keep them strictly separate.
            
        best_beam_pairs_list_rank1 = []
        for i in range(len(best_beam_pairs)):
            best_beam_pairs_list_rank1.append(int(best_beam_pairs[i].beam_idx)) 
          
        
        if n_csi_beams == 1:
            #If Ls=1, then you dont need a criss cross, automatically done.    
            ...
            if not l:  
                ...
                # Relative power index saved for further orthogonality checks
                 ...
                 Fin_w_op1 = np.add((codebook_subset[:, best_beam_relative_idxs[0]]), \
                       (amp1 * (codebook_subset[:, best_beam_relative_idxs[1]])))
                    
                 Fin_w_op1 = (1/np.sqrt(1+(amp1 * amp1)))*(Fin_w_op1)
        ...
        if not n_csi_beams == 1:
            if not l:
                # SAVING OPTION 1 CURR BEAM PAIR SET!!!!
                    # Compute internal product between ch coeffs and precoder, 
                    # that is what the UE will see from a transmission with w
                    # Fin_at_ue_ant = np.dot(mean_coeffs[l], Fin_w)
                 Fin_bs_weights_op1 = Fin_w_op1 / np.linalg.norm(Fin_w_op1)
                 Fin_at_ue_ant_op1 = np.dot(mean_coeffs[l], Fin_bs_weights_op1)
                    # The UE will use the Maximum Ratio Beamformer, 
                    # both for receiving and for transmitting
                 Fin_mr_precoder_temp = Fin_at_ue_ant_op1.conj().T
                 Fin_mr_precoder_op1 = Fin_mr_precoder_temp / np.linalg.norm(Fin_mr_precoder_temp)
                        
                    # Resulting in a amplitude channel gain of:
                 Fin_ch_gain_op1 = abs(np.dot(Fin_at_ue_ant_op1, Fin_mr_precoder_op1))
                 power_per_beam_op1 = Fin_ch_gain_op1
                    # Create and load the best Beam Pair found
                    
                 created_beam_pair_op1.bs_weights = Fin_bs_weights_op1
                    # Sandra-print('Fin_bs_weights: ',Fin_bs_weights)
                 created_beam_pair_op1.ue_weights = Fin_mr_precoder_op1
                    # Save the channel gain (linear/electric field)**2 = power gain
                 created_beam_pair_op1.ch_power_gain = Fin_ch_gain_op1 ** 2
                
                 curr_beam_pairs[(bs,ue,0,l)] = created_beam_pair_op1
            else:
                    # Compute internal product between ch coeffs and precoder, 
                    # that is what the UE will see from a transmission with w
                    # Fin_at_ue_ant = np.dot(mean_coeffs[l], Fin_w)
                ...
                #Saving for rank 1 or second layer, we can try to make this 
                #code cleaner once validation is through, this can be made by
                #putting the below two blocks into a for loop and making it one.
                Fin_bs_weights_rank1_op2 = Fin_w_rank1_op2 / np.linalg.norm(Fin_w_rank1_op2)
                Fin_at_ue_ant_rank1_op2 = np.dot(mean_coeffs[0], Fin_bs_weights_rank1_op2)
                # The UE will use the Maximum Ratio Beamformer, 
                # both for receiving and for transmitting
                Fin_mr_precoder_temp_rank1 = Fin_at_ue_ant_rank1_op2.conj().T
                Fin_mr_precoder_rank1_op2 = Fin_mr_precoder_temp_rank1 / \
                                            np.linalg.norm(Fin_mr_precoder_temp_rank1)
                            
                # Resulting in a amplitude channel gain of:
                Fin_ch_gain_rank1_op2 = abs(np.dot(Fin_at_ue_ant_rank1_op2, \
                                                   Fin_mr_precoder_rank1_op2))
                power_per_beam_rank1_op2 = Fin_ch_gain_rank1_op2
                # Create and load the best Beam Pair found
                        
                created_beam_pair_rank1_op2.bs_weights = Fin_bs_weights_rank1_op2
                # Sandra-print('Fin_bs_weights: ',Fin_bs_weights)
                created_beam_pair_rank1_op2.ue_weights = Fin_mr_precoder_rank1_op2
                # Save the channel gain (linear/electric field)**2 = power gain
                created_beam_pair_rank1_op2.ch_power_gain = Fin_ch_gain_rank1_op2 ** 2
                    
                curr_beam_pairs[(bs,ue,1,0)] = created_beam_pair_rank1_op2
                     
                     
                #Saving for rank 2 or second layer, we can try to make this 
                #code cleaner once validation is through
                Fin_bs_weights_rank2_op2 = Fin_w_rank2_op2 / np.linalg.norm(Fin_w_rank2_op2)
                Fin_at_ue_ant_rank2_op2 = np.dot(mean_coeffs[1], Fin_bs_weights_rank2_op2)
                # The UE will use the Maximum Ratio Beamformer, 
                # both for receiving and for transmitting
                Fin_mr_precoder_temp_rank2 = Fin_at_ue_ant_rank2_op2.conj().T
                Fin_mr_precoder_rank2_op2 = Fin_mr_precoder_temp_rank2 / \
                                            np.linalg.norm(Fin_mr_precoder_temp_rank2)
                            
                # Resulting in a amplitude channel gain of:
                Fin_ch_gain_rank2_op2 = abs(np.dot(Fin_at_ue_ant_rank2_op2, \
                                                   Fin_mr_precoder_rank2_op2))
                power_per_beam_rank2_op2 = Fin_ch_gain_rank2_op2
                # Create and load the best Beam Pair found
                        
                created_beam_pair_rank2_op2.bs_weights = Fin_bs_weights_rank2_op2
                # Sandra-print('Fin_bs_weights: ',Fin_bs_weights)
                created_beam_pair_rank2_op2.ue_weights = Fin_mr_precoder_rank2_op2
                # Save the channel gain (linear/electric field)**2 = power gain
                created_beam_pair_rank2_op2.ch_power_gain = Fin_ch_gain_rank2_op2 ** 2
                    
                curr_beam_pairs[(bs,ue,1,1)] = created_beam_pair_rank2_op2
...
def orthogonal_precoder_indices1(N1, N2, O1, O2, RI, q, q1=-1, q2=-1):
    ...
    #Now both the rank 1 and rank 2 will have exact same precoders, but lets 
    #check if this stacking is indeed needed.
    if RI == 2:
        q_idxs = np.hstack((q_idxs, q_idxs))
...
def su_mimo_setting_bitrate_single_layer(bs, ue, n_prb,
    ...
    beam_pair = curr_beam_pairs[(bs, ue, 0, 0)]
...
def su_mimo_setting_bitrate_dual_layer(bs, ue, n_layers, n_prb,
    ...
    for l in range(n_layers):
        beam_pair = curr_beam_pairs[(bs, ue, 1, l)]
        signal_power = tx_pow_dl_per_layer * beam_pair.ch_power_gain
        ...
        #NEED TO CROSS VERIFY THE ESTIMATED INTERFERENCE USED FOR THE LAYER!!    
...
def is_compatible_with_schedule(new_entry, schedule, beam_dist_lim, n_csi_beams):
    ...
    for schedule_entry in schedule:
        ...
        #For rank 2, remove the above if statement, also in below make sure every
        #layer is checked for every UE, if yes then this is enough. Schedule entry
        #has a layer index also? Please check that.
...
############### CSI UPDATE WRAPPERS ##############
# def beam_RPI_book_keeping(ues, n_layers, tti, curr_beam_pairs)
...
def interference_measurements_final_update(ues, n_layers, tti, last_csi_tti, 
                                     csi_tti_delay, est_fin_interference,
                                     real_interference, curr_beam_pairs,
                                     curr_schedule, book_keeping_interference, 
                                     book_keeping_dict, interference_dict,
                                     beam_entry):
    
    #beam entry tuple creation, this should be created from the info in 
    #in curr_beam_pairs, but with the filtered ones that are scheduled.
    #So you need to have a combination of curr_schedule and curr_beam_pair here
    ...
    if book_keeping_interference is False:
        for ue in ues:
            for p in range(n_layers):
                ...
                t = last_csi_tti - csi_tti_delay
                est_fin_interference[tti][ue][p] = real_interference[t][ue][p]
                
                # however, if the measurement is null, use a past measurement
                if est_fin_interference[tti][ue][p] == 0:
                    # last_non_zero = np.max(np.nonzero(est_interference[:][ue][p]))
                    est_fin_interference[tti][ue][p] = est_fin_interference[tti-1][ue][p]
    else:
    # New interference estimation mechanism, if there is a perfect match with
    # the book keeping table wrt ue-beams_used-RPI of the beams_used, then
    # copy the interference of that entry to the est interference, else use the
    # same method as before.    
        i = 0
        fin_entry = perfect_match_book_keeping(beam_entry, book_keeping_dict)
        if  fin_entry is not False:
            # Assign the interference from fin_entry to the respective ues
            ue_list = [i[0] for i in beam_entry[0]]
            layer_list = [i[1] for i in beam_entry[0]]
            for ue, p in zip(ue_list, layer_list):
                # for p in range(est_scheduled_layers[tti][ue]):
                est_fin_interference[tti][ue][p] = interference_dict[fin_entry][ue][p]
                i = i + 1
...
def perfect_match_book_keeping(beam_entry, book_keeping_dict): 
    
    counter = 0
    
    # Make sure that in below loop every entry means every key, since for every
    # new entry we have a new key, so it should loop across all keys!!!!!!
    for entry in book_keeping_dict:
        if entry == beam_entry :
            counter = counter + 1
            fin_entry = entry
...
def mu_mimo_choice(tti, curr_priorities, curr_schedule, serving_BS_dl, 
                   ...
                   real_scheduled_layers, debug, n_csi_beams, n_layers):
    ...
    for (ue, priority) in curr_priorities:
        ...
        for l in range(est_scheduled_layers[tti][ue]):
            ...
            if n_layers == 1:
                beam_pair = curr_beam_pairs[(bs,ue,0,l)]
            else:
                beam_pair = curr_beam_pairs[(bs,ue,1,l)]
...
def interference_book_keeping_update(curr_schedule, curr_beam_pairs, n_layers,
                                     est_scheduled_layers, book_keeping_dict):
    
    book_keeping_dict_entry_ue = []
    book_keeping_dict_entry_precoder = []
    book_keeping_dict_entry_RPI = []
    book_keeping_dict_entry = ()
    
    
    for i in range (len(curr_schedule['DL'])):
        x = curr_schedule['DL'][i].ue
        l = curr_schedule['DL'][i].layer_idx
        xl = [x,l]
        book_keeping_dict_entry_ue.append(xl)
        #CHANGE THE BS AND L IN THE BELOW CODE TO MAKE IT MORE GENERALISED!!!
        #if len(curr_beam_pairs[0,x,0].beam_idx) == 1:
        # for l in range est_scheduled_layers[tti][ue]
        if n_layers == 1:
            ra = 0
        else:
            ra = 1
        if(isinstance(curr_beam_pairs[0,x,ra,l].beam_idx, numpy.int32)):
            book_keeping_dict_entry_precoder.append([curr_beam_pairs[0,x,ra,l].beam_idx])
            book_keeping_dict_entry_RPI.append([curr_beam_pairs[0,x,ra,l].RPI])
        else:
            book_keeping_dict_entry_precoder.append(curr_beam_pairs[0,x,ra,l].beam_idx)
            book_keeping_dict_entry_RPI.append(curr_beam_pairs[0,x,ra,l].RPI)
                   
    # Sort the UE list in ascending order and reordering the precoder and RPI,
    # This is to remove duplicates in the beam entry list due to order change 
    # of ue listing
    book_keeping_dict_entry_precoder = [x for _,x in sorted(zip(book_keeping_dict_entry_ue, 
                                         book_keeping_dict_entry_precoder))]
    book_keeping_dict_entry_RPI = [x for _,x in sorted(zip(book_keeping_dict_entry_ue, 
                                         book_keeping_dict_entry_RPI))]
    book_keeping_dict_entry_ue = sorted(book_keeping_dict_entry_ue)
    
    # converting above list to tuples before appending them as an entry to dict        
    # book_keeping_dict_entry_ue = tuple(book_keeping_dict_entry_ue)
    book_keeping_dict_entry_ue = \
                tuple(tuple(sub) for sub in book_keeping_dict_entry_ue)
    book_keeping_dict_entry_precoder = \
                tuple(tuple(sub) for sub in book_keeping_dict_entry_precoder)
    book_keeping_dict_entry_RPI = \
                tuple(tuple(sub) for sub in book_keeping_dict_entry_RPI)
                
    book_keeping_dict_entry = (book_keeping_dict_entry_ue,) + \
                                book_keeping_dict_entry_precoder + \
                                    book_keeping_dict_entry_RPI
    # book_keeping_dict[book_keeping_dict_entry] = book_keeping_dict_entry
    
    return(book_keeping_dict_entry)
...
def tti_simulation(curr_schedule, slot_type, n_prb, debug, coeffs, 
    ...
    beam_entry = tuple(beam_entry)
    beam_entry_list.append(beam_entry)
    # print(f'_____Entry {beam_entry}: '
    #               f'TTI {tti}')
    book_keeping_dict[beam_entry] = beam_entry
    interference_dict_entry = ()  
    ...
    for entry in curr_schedule[slot_type]:
        ...
        # beam_learning[tti][entry.ue][entry.layer_idx] =  \
        #                 real_dl_interference[tti][entry.ue][entry.layer_idx]
        # print('First 5 SINRs: ' + 
        ...
        #           f'{realised_bitrate[tti][entry.ue][entry.layer_idx]} '
        #           'Mbits/s') 
    interference_dict_entry = real_dl_interference[tti]
    interference_dict[beam_entry] = interference_dict_entry
